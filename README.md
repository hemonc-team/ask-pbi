...
1. Появится **ссылка и код** — откройте ссылку в браузере, вставьте код, войдите **рабочим email** Power BI.
2. Не закрывайте окно команды: оно само дождётся входа и напишет `OK, вход сохранён.`
3. Проверка — должен появиться список ваших рабочих областей:
...
|---|---|
| Claude не видит `~/ask-pbi` / предлагает другой skill | Вы в **Chat**. Откройте **Cowork** → **на этом компьютере** |
| Claude пишет, что нет интернета / облако / песочница | Cowork, режим **на этом компьютере** (не облако) |
